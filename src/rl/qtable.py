...
class QTable(object):
    ...
    def __call__(self, state: Environment, action) -> np.float64:
        return self.get(state, action)

    def get(self, state: Environment, action) -> np.float64:
        return self._q_definition.get((state, action), 1.0)

    def update(self, state: Environment, action, value: np.float64) -> None:
        key = (state, action)
...
class TrackingQTable(QTable):
    ...
    def update(self, state: Environment, action, value: np.float64):
